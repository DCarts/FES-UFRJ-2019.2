...
public class Professor {
  ...
  public Professor(String nome, String cpf, String email, String endereco, Date data_nascimento) {
    this.nome = nome;
    this.cpf = cpf;
    this.email = email;
  ...
  public boolean save() throws SQLException {
    ...
    if (rs.next()) {
      ...
      PreparedStatement statement3 = conn.prepareStatement("INSERT INTO professor VALUES (?)");
  ...
  public static ArrayList<Professor> all() throws SQLException {
    ...
    while(rs.next()) {
      result.add(new Professor(rs));
    }
    return result;
  }

  public static Professor findById(String cpf) throws SQLException {
    Connection conn = Conexao.getConnection();
    PreparedStatement statement = conn.prepareStatement("SELECT * FROM pessoa WHERE cpf=?");
    statement.setString(1, cpf);
    ResultSet rs = statement.executeQuery();
    if (rs.first()) {
      return new Professor(rs);
    }
    return null;
  }
}